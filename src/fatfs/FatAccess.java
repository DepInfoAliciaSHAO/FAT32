...
public class FatAccess{
    ...
    private IDevice device; //le device dans lequel on lit et écrit, il s'agit d'un IDevice car il peut sagir d'un device out d'un ramdevice
    private int beginFatSector; //le premier secteur de la premiere fat
    private int sizeSector; //la taille de secteur 512 en général
    private int sizeCluster; // la taille de Cluster en secteur, 2 secteur en général
    private int totalNumberSector; // le nombre total de secteur sur le disque
    private int fatNumberOfCase; // la fat prend 4 octet par cluster donc on fait numberCluster*4octer)//sizeSector +1
    private int numberOfFat; //le nombre de fat (redondance pour ne pas perdre de donnée) 2 en général
    private int sectorPerFat; // le nombre de secteur neccessaire pour stocker une fat
    private int rootIndex; //l'index de root dans la fat, 2 en général
    ...
    * @param device Device from which the FAT is read.
    ...
    public void write(int data, int index){
        int byteIndex = 4*index; //idem commentaire que read
        int indexSector = byteIndex/sizeSector;
        int indexInSector = byteIndex%sizeSector;
        byte[] sector = device.read(beginFatSector + indexSector);
    ...
    public void remove(int index){
        write(0x00000000, index); //on écrit la valeur signifiant que le secteur est vide
    }
    ...
    private int readBytes(byte[] sector, int index, int size){
        ...
        for (int i = 0; i < size; i++) {
            res |= (sector[i + index] & 0xFF) << ((size - 1 - i) * 8); //vous occupez pas des calculs
        } //mais grosso modo & 0xFF c'est pour regler les problème de signe parceque les byte sont signé et nous on veux pas
        return res; //et << c'est pour décaler d'un octer
    }
