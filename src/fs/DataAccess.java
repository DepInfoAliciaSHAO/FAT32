...
public class DataAccess{
    ...
    private IDevice device; //le device pour les lecture de secteur
    private FatAccess fatAccess; //fatAcees pour les lecture et écriture dans la fat ainsi que les information de formatage
    private int beginDataSector; //l'indice du premier secteur de la zone de data
    ...
    public Vector<DataFile> readSubFile(DataFile directory){
        ...
        if (!directory.getAttribut()[4]){
        ...
        else { //sinon c'est qu'on a un dossier
            int index = directory.getFirstClusterIndex(); //c'est deux variable sont utilisé pour
            int nextIndex = index; //savoir quand est ce qu'on arrive au dernier cluster
            Vector<DataFile> res = new Vector<DataFile>();
            ...
            do { //Iteration over the cluster-index-linked list in the FAT.
                ...
                nextIndex = fatAccess.read(index); //on lit l'indice du prochain cluster
            } while (nextIndex != 0x0FFFFFFF); //on verifie si c'était le dernier cluster
            return res;
        }
    }
    ...
    public boolean[] readBinaryFileContent(DataFile file) {
        //TODO tester impérativement cette méthode
        if (file.getAttribut()[4]) {
            return new boolean[0];
        } else { //sinon c'est un fichier
            int index = file.getFirstClusterIndex(); // index et nextIndex sont les index des fichiers actuellement
            int nextIndex = index; // lut, cela permet de savoir quand s'arrêter en lisant la fat
            int size = file.getSize(); //taille du fichier, pour savoir quand s'arrêter
            boolean[] result = new boolean[size]; //réponse un tableau de booléens de taille size qu'on convertira en tableau de byte
            int sizeAct = 0; // taille lue actuellement, vérifie qu'on ne lit pas plus loin que la taille allouée
            do { // on itère sur la liste chaînée
                index = nextIndex; //on actualise index
                for (int j = 0; j < fatAccess.getSizeCluster(); j++) { // on itère sur la taille des clusters
                    byte[] sector = device.read(beginDataSector + j + index * fatAccess.getSizeCluster()); //on calcule l'indice du secteur actuel en fonction de la taille des clusters
                    for (int i = 0; i < fatAccess.getSizeSector(); i++) { // on itère sur la secteur
                        if (sizeAct == size) { //on s'arrête si on a lu la bonne quantité d'informations
                            break;
                        } else {
                            result[sizeAct] = (sector[i] != 0); // conversion de byte à boolean
                            sizeAct++;
                        }
                    }
                }
                nextIndex = fatAccess.read(index); // il y a un problème là avec la condition d'arrêt
            } while (index != 0x0FFFFFFF); //signe que c'était le dernier maillon
            return result; //on renvoie result
        }
    }
